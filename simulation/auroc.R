...
readr::write_csv(data.frame(ypred = y.pred, y = as.numeric(ytest)), "XGBoostPredSim.csv")
...
myrocT5 = pROC::roc(predTpotDsSim$y, predTpotDsSim$ypred)
...
auc.plot <- ggplot(data = df, aes(x = Specificity, y = Sensitivity, color = Type))+
  ...
  annotate("text", x = 0.52, y = 0.25, label = paste0('TPOT AUC: ', round(myrocTStandard$auc, 3)), color = cbPalette[1])+
  annotate("text", x = 0.68, y = 0.9, label = paste0('TPOT-DS AUC: ', round(myrocT5$auc, 3)), color = cbPalette[2])+
  annotate("text", x = 0.3, y = 0.6, label = paste0('XGBoost AUC: ', round(myrocX$auc, 3)), color = cbPalette[3]) +
  guides(color=FALSE)
auc.plot
ggsave(auc.plot, filename = "aucSim.svg", height = 5, width = 5)
